...
目录 基于多线程的电梯调度系统一、项目分析1. 项目背景2. 项目目的3. 功能需求二、开发工具三、术语表四、设计1. 总体算法设计2. 调度算法设计(1)外调度算法(2)内调度算法3. 数据结构设计(1)主要变量设计(2)主要函数设计五、项目实现1. 具体实现(1)总体算法核心函数(2)外调度算法核心函数(3)内调度算法实现2. 项目界面(1)电梯系统界面(2)电梯界面(3)电梯内部面板图(4)电梯开门示意图(5)电梯运行状态示意图#六、提交内容
...
本项目需要实现多电梯调度机制。对于多部电梯的外请求而言，本项目采用外调度算法，以调度电梯响应该请求；对于每部电梯的内请求而言，本项目采用内调度算法，以响应电梯内部的请求队列。
...
本项目了解到现实情况中人们对电梯的使用需求，采用了以顺路级别为标准的优先级调度算法，同时考虑到原优先级调度算法可能导致饥饿现象的出现，提出了结合老化算法的优先级调度算法。

- 调度算法：结合老化的优先级调度算法
  - 优先级调度算法原理：
    优先级调度：   
    		以顺路级别判定——顺路级别越高，优先级越高，最优者响应该外请求。
    顺路级别：
...
根据需求分析，可以设计电梯系统界面，界面分为5个部分，每个部分对应一个电梯。如下：
...
电梯内部面板界面分为以下几个部分：显示楼层、显示上/下行状态、紧急键、开门键、关门键与内请求楼层键。如下：
